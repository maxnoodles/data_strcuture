class SegmentTree:
    ...
    def set(self, index, val):
        if not 0 <= index < len(self._data):
            raise ValueError('Index is illegal')
        self._set(0, 0, len(self._data) - 1, index, val)

    def _set(self, tree_index, l, r, index, val):
        if l == r:
        ...
        r_index = self._right_child(tree_index)
        if index <= mid:
            self._set(l_index, l, mid, index, val)
        else:
            self._set(r_index, mid+1, r, index, val)
...
if __name__ == "__main__":
    ...
    print(segment_tree)
    query_result = segment_tree.query(0, 8)
    print(query_result)

    segment_tree.set(0, 111)
    print(segment_tree)
    query_result = segment_tree.query(0, 8)
    print(query_result)
